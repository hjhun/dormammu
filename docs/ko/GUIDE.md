...
두 단계 모두 opt-in 방식으로, `agents.refiner.cli` / `agents.planner.cli`가
명시적으로 설정된 경우에만 실행됩니다.
...
기반 파이프라인을 사용합니다. `refiner`와 `planner`는 opt-in으로, `cli`가
명시적으로 설정된 경우에만 실행됩니다.
...
refiner (opt-in) → planner (opt-in) → developer → tester → reviewer → committer
         ▲                                 ▲              │           │
         └── REQUIREMENTS.md 작성          └── FAIL ──────┘           │
                  │                        ▲                          │
         WORKFLOWS.md 작성 (planner)       └──────── NEEDS_WORK ──────┘
```
...
**Refiner** (opt-in): raw prompt를 읽어 `.dev/REQUIREMENTS.md`에 범위,
수용 기준, 제약 사항, 리스크를 정리합니다. `agents.refiner.cli`가 명시적으로
설정된 경우에만 실행됩니다.

**Planner** (opt-in): `.dev/REQUIREMENTS.md` (또는 raw prompt)를 읽어
`.dev/WORKFLOWS.md` — 적응형 `[ ] Phase N.` 체크리스트를 작성합니다.
`PLAN.md`와 `DASHBOARD.md`도 갱신합니다. `agents.planner.cli`가 명시적으로
설정된 경우에만 실행됩니다.
...
`refiner`와 `planner`는 fallback 없이 **명시적으로 설정된 경우에만** 실행됩니다.
기존 단일 CLI 설정에는 영향을 주지 않습니다.
