...
기반 파이프라인을 사용합니다. `analyzer`는 goals 자동화에서 사용되고,
`refiner`와 `planner`는 필수 런타임 단계로 항상 실행되며 역할별 CLI가
없으면 `active_agent_cli`를 사용합니다.
...
```
analyzer (goals prelude) → planner (goals prelude) → architect (optional)
...
- 필요하면 `architect`가 기술 설계 문맥을 추가

그 다음 큐에 들어간 prompt는 런타임 파이프라인이 반드시
`refine -> plan`부터 시작하고, 이후 단계는 `.dev/WORKFLOWS.md`를 통해
planner가 결정하도록 지시합니다.
