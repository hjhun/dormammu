...
이미 `Python 3.11` 이상이 보이면 이 문서는 건너뛰고 일반 설치 절차를
진행하면 됩니다.
...
- Ubuntu 24.04 이상:
  ...
  진행해도 됩니다.
- Ubuntu 22.04:
  `python3.11` 패키지를 설치해서 진행하면 됩니다.
- Ubuntu 20.04 이하:
  기본 저장소에 원하는 버전이 없을 수 있으므로 `deadsnakes` PPA 같은
  별도 패키지 소스를 사용하거나, 가능하면 OS 업그레이드를 먼저 검토하는
  편이 좋습니다.
...
그 다음 Python 3.11과 가상환경 패키지를 설치합니다.
...
Ubuntu 20.04처럼 기본 저장소에 `python3.11`이 없는 환경에서는 PPA가
필요할 수 있습니다.
...
이후 Python 3.11과 가상환경 패키지를 설치합니다.
...
여기서 `python --version` 결과가 `3.11+`로 나오면 정상입니다.
...
### `python3.11-venv`가 없어서 `venv` 생성이 실패할 때
...
`python3.11`로 가상환경을 만들고 그 환경에서 `dormammu`를 실행하는
것입니다.
