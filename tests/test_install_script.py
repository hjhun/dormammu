...
class InstallScriptTests(unittest.TestCase):
    ...
    def test_local_install_script_upgrades_build_backend_before_editable_install(self) -> None:
        script = (ROOT / "setup.sh").read_text(encoding="utf-8")
        build_backend_index = script.index("ensure_build_backend")
        pep517_index = script.index('"${VENV_DIR}/bin/python" -m pip install --use-pep517 --upgrade "$(editable_spec)"')

        self.assertLess(build_backend_index, pep517_index)
        self.assertIn('"${VENV_DIR}/bin/python" -m pip install --upgrade "setuptools>=68" wheel', script)
